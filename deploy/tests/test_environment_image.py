...
from deploy.tests import test_environment_artifact as artifact_tests
from deploy.environments.artifact import build_artifact, tree_hashes
...
class EnvironmentImageTests(unittest.TestCase):
    def candidate(self, root):
        fixture = artifact_tests.EnvironmentArtifactTests()
    ...
    def test_build_uses_archived_source_and_binds_image_to_expected_artifact(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            def fake_build(context, output, labels):
                ...
                self.assertEqual((context / 'VERSION').read_text().strip(), manifest['version'])
                self.assertIn('ENV APP_VERSION=0.28.15', (context / 'Dockerfile').read_text())
            ...
            with patch('deploy.environments.image.docker_build', side_effect=fake_build), \
                    patch('deploy.environments.image.image_files', side_effect=lambda image:
                          tree_hashes(root / 'image/context/backend')), \
                ...
                for field, value in [('version', '9.9.9'), ('commit', 'f' * 40),
                                     ('image_id', 'mutable:tag'), ('labels', {})]:
                    (root / 'image/image.json').write_text(json.dumps({**result, field: value}))
                    with self.subTest(field=field), self.assertRaisesRegex(ValueError, 'image_manifest_invalid'):
                        verify_image(root / 'artifact', manifest['artifact_id'], root / 'image')
                (root / 'image/image.json').write_text(json.dumps(result))
                inspect.return_value['Config']['Env'] = ['APP_VERSION=0.0.0']
                with self.assertRaisesRegex(ValueError, 'image_identity_mismatch'):
                    verify_image(root / 'artifact', manifest['artifact_id'], root / 'image')
                inspect.return_value['Config']['Env'] = ['APP_VERSION=' + manifest['version']]
                (root / 'image/context/backend/init.sql').write_text('unexpected image content')
                with self.assertRaisesRegex(ValueError, 'image_content_mismatch'):
                    verify_image(root / 'artifact', manifest['artifact_id'], root / 'image')

    def test_build_rejects_docker_output_with_wrong_runtime_version(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            manifest = self.candidate(root)
            with patch('deploy.environments.image.docker_build', return_value='sha256:' + 'a' * 64), \
                    patch('deploy.environments.image.inspect_image', return_value={'Id': 'sha256:' + 'a' * 64,
                          'Config': {'Labels': {}, 'Env': ['APP_VERSION=0.0.0']}}):
            ...
            self.assertTrue((root / 'image/failure.json').is_file())
