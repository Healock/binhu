...
The login page has no manual Staging switch. A complete username ending in
`@staging` selects the fixed `/staging/api` route after the server bootstrap
identity is verified; a failed or mismatched Staging connection never falls
back to the production `/api` route.
