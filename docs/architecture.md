...
业务数据导入同时提供“快捷下发（全链条）”入口，供内勤针对一两条临时数据手工创建任务。快捷下发会校验身份证号、启用社区、业务日期和截止日期，在同一事务中创建一个已审核、待发布的下发批次与任务；它不会直接写入腾讯表，仍需在下发工作台完成发布、回读和审计。快捷批次不保存原始文件，批次和任务的标准字段与 Excel 导入保持一致，便于后续重试、对账和历史追溯。
