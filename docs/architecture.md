...
> 当前基线（v0.20.7）：平台仍使用同一个 MySQL 实例和八个业务域数据库；任务处理下统一提供“下发任务处理、研判、调照片”三个入口。“研判”可切换待研判与已研判，并使用独立详情和保存接口。`online.task.manage` 只在流口任务工作台内授予全所查看、现有任务编辑和核查人分配，不开放 Univer 或腾讯原始行增删。业务和管理列表使用统一的即时筛选工具栏。“工单中心”位于工作台，调照片处理队列使用独立页面。快捷调照片工单完成后，原核查任务详情直接投影可访问的照片附件，并在任务卡片标记“已调照片”；导入照片附件以“姓名_身份证号”作为下载名称。在线数据查询的 Univer 工作表仅向管理员和超级管理员开放；流口任务桌面端支持账号级卡片/双层表格视图，两种视图在电脑端互斥显示，表格当前页会批量准备可填写字段并允许在第二行直接编辑，每条任务的上下两行使用圆角卡片分组且上下层外框对齐，来源字段与卡片视图统一显示为标签云，手机端保持卡片且枚举下拉不再唤起输入法。下发批量分配按每批 20 条处理并持续显示互斥的成功、跳过和失败统计，重试不会覆盖已成功任务。走访明细和新星级评分管理支持从仅服务器可访问的旧平台执行受保护的只读预览；联合预览会先投影同批走访，再计算星级匹配。房东责任告知书读取改为持久化后台任务，逐页保存进度并允许刷新页面后继续观察，失败时可从已验证断点继续；完成后清理临时分页正文。固定生产发布会记录上传接收、校验解压、预检、镜像构建、备份、切换、容器重建和健康检查的独立耗时。全民防模型三仍仅提供账号 `shenshenghua` 的单条“在吴”只读预演，且生产开关受登录协议、授权设备及目标机构配置共同约束；超级管理员可在系统设置中录入接口、账号、密码和授权设备值，密码加密保存且不回显，IMEI/MACHINEUID按授权要求完整显示；该版本没有登记、反馈或批量写入能力。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
