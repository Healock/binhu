...
> 当前基线（v0.19.9）：平台仍使用同一个 MySQL 实例和八个业务域数据库；调照片名单支持腾讯新增申请增量进入平台、平台快捷申请提交后立即异步写回腾讯、完整核对安全重定位，以及腾讯批次结单后继续通过 ZIP 补挂真实照片。工单附件查看与管理权限已经分离：有权访问工单的业务人员可预览和下载附件，只有处理岗位和工单管理员可上传或删除。小区管理支持组长、组员维护和导出本人社区资料；已处理下发文件必须先预览确认再正式落库。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
