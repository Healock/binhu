...
全链条当前只支持“预览—确认—导出即归档”的反馈闭环。离苏、无需登记和移交（所外）可直接进入候选；历史“移交”以及移交（所内）由基础管控先明确审核决定；截止日期后的无法核实和已登记数据分别按审核决定、最近一次确认的公安网原始数据进行判断。导出工作簿先安全写入持久化目录并记录哈希，随后由后台任务按腾讯物理行号从大到小删除整行。任何来源版本、行哈希、实时内容或同步刷新冲突都只记为冲突，不会把任务静默从列表中隐藏。导出文件和公安网原始数据快照均保留在 `BINHU_FULLCHAIN_ARCHIVE_DIR`，可从历史记录重新下载。

公安网原始上传只保存身份证 HMAC 摘要用于比对，原始文件本身用于审计和重新下载。存在重复来源或投影冲突的任务会明确标记为不可归档，必须先完成来源核查；后台任务中断后冻结为部分完成，已生成的反馈文件不回滚。
