...
“数据工作台 → 工作日志生成”把系统统计和管理员填写的内容合并为 DOCX。第一阶段只支持日报：

- `OnlineData._work_log_drafts` 按“日志类型＋业务日期”唯一保存草稿。
- 创建草稿时读取一次模型三、出租房走访和自购房走访数据，并把结果保存为系统快照。以后底层统计变化不会悄悄改变草稿；只有当前编辑人点击“刷新系统数据”才重新读取。
...
- 模型三固定使用两列口径：三列中的“已完成”作为工作日志的“已核查”，其余两种未完成状态合并为“未核查”。这与账号个性化显示方式无关。
- 所选日期没有对应日报或没有任何走访数据时，系统字段保持空白并说明来源不可用，不能用 0 伪装成已确认的统计结果。

日报母版是脱敏的 `daily-v1.docx`。导出时临时生成 12 个内嵌 XLSX 和对应预览，写入 DOCX 后直接返回，不在服务器保存生成文件。母版、工作簿、预览图、链接和文档属性都不能包含旧样例中的身份或警情信息。业务日期决定取数和文件名，首页落款使用下一自然日。
...
- 人员当天跨社区走访时，这 1 个人日按各社区当天的走访户数比例分配；当天没有走访时，
  人日归到人员管理中的所属社区。名册外人员只有在真实产生走访的日期才补 1 个人日。
