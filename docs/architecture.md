...
人员可以跨社区走访，因此操作人的所属社区和本次走访社区不同不算异常，也不会产生提醒。
...
- 社区中的网格员人数按所选区间内、在该社区实际有走访记录的不同操作人计算。
- 人均走访户数等于走访户数除以网格员人数；人均变动数等于总变动数除以网格员人数，
  两项都四舍五入保留 1 位小数。
- 星级评定数只计算已经成功关联的走访，星级评定率等于星级评定数除以走访户数。
- 表底总计会按整个区间内不重复的操作人数，重新计算两个人均指标、户均变动数和星级评定率；
  同一个人跨社区走访时不会在总计人数中重复计算。
...
目前每名人员只保存一个临时请假时间段。多次请假和外部系统对接的改造方向见
...
单日工作量先对比前后两天快照，找出当天新增或地址、核查结果发生变化的数据，再统一按照
当天最终状态归类：地址和结果都未填写是“未核查”，只填写地址是“已核查”，填写了核查
结果就是“已完成”。因此，从在线表格第一次同步进来时就已经填写“已登记”或“无法核实”
的数据，会进入“已完成”；原本已经填写地址、当天只是修正地址内容的数据，仍会进入
“已核查”。单日报表中的未核查、已核查、已完成三列之和必须等于数据总数。

已经存在且当天完全没有变化的数据不会进入单日工作量。这些数据仍保存在原始数据和快照中；
如果要查看“截至当天仍有多少未完成”，需要另行增加存量视图，不能把它和当天工作量混为
一张表。
...
数据的人不会被隐藏。社区汇总再根据这张合并后的网格员明细计算，避免两种口径不一致。
