...
- 滨湖平台12个正式社区按2026-08-17确认的苏州居住证平台清单提供默认代码。启动迁移只补空值，管理员已保存的非空值不会被覆盖。
...
- `PlatformData._communities.qmf_community_code` 保存社区在全民防中的10位大写字母/数字代码；社区管理页负责维护。启动迁移会按已核验的12社区清单仅补空值，不覆盖管理员后续修改。任务地址到社区继续复用 `RegistryData._police_address_entries` 小区管理数据，不另建第二套映射。
