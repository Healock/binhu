...
个人任务依赖图由 Rete.js 展示真实任务实例，不保存第二份业务正文。独立任务仍从各业务域实时读取；任务首次产生前置关系时，`WorkflowData` 只保存节点引用、依赖状态和安全事件。首期规则为“基础管控研判 → 原网格员核查任务”：无法核实会阻塞原任务，研判完成后解锁；同一节点在负责人视角可处理，在协作方视角只读。只有整条依赖链全部终结后才进入历史。
