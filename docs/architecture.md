...
> 当前基线（v0.19.27）：平台仍使用同一个 MySQL 实例和八个业务域数据库；任务处理下统一提供“下发任务处理、研判、调照片”三个入口。“研判”可切换待研判与已研判，并使用独立详情和保存接口。`online.task.manage` 只在流口任务工作台内授予全所查看、现有任务编辑和核查人分配，不开放 Univer 或腾讯原始行增删。业务和管理列表使用统一的即时筛选工具栏。“工单中心”位于工作台，调照片处理队列使用独立页面。快捷调照片工单完成后，原核查任务详情直接投影可访问的照片附件，并在任务卡片标记“已调照片”；导入照片附件以“姓名_身份证号”作为下载名称。在线数据查询的 Univer 工作表支持选择任意当前列进行后台全量升序或降序，排序与筛选、关键词共同生效，不在浏览器内重排行号或腾讯来源。数据上传中心只负责上传、预览和导入；照片批次预览的前端、Nginx 与后端统一支持最大 200MB ZIP，并保留 multipart 传输余量。下发任务工作台统一承载审核与多选发布，支持逐卡选择和覆盖所有分页的筛选全选，并为当前批次提供明确的历史详情入口。走访明细和新星级评分管理支持从仅服务器可访问的旧平台执行受保护的只读预览；联合预览会先投影同批走访，再计算星级匹配，避免尚未确认的走访被错误显示为全部未匹配。认证信息只保存在生产环境变量，默认仍需人工确认后才替换数据，原 XLSX 上传保留兜底。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
