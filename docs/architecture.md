...
> 当前基线（v0.21.9 功能分支）：平台仍使用同一个 MySQL 实例和八个业务域数据库；既有任务处理、流口工作台、走访来源、告知书读取和固定发布结构保持不变。全民防模型三仅向账号 `shenshenghua` 提供一个“全民防登记”入口；在吴和近期返吴执行九步人员登记，离开不返吴执行四步注销反馈。每次准备和执行前通过旧管理平台复核是否已经反馈，运行记录仍只保存安全摘要和步骤状态。
