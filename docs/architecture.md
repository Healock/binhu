...
> 当前基线（v0.19.13）：平台仍使用同一个 MySQL 实例和八个业务域数据库；任务处理下统一提供“下发任务处理、研判、调照片”三个入口。“研判”是统一工作台，默认处理支持“无法核实”结果的流口业务中，网格员已填写“无法核实”但尚未填写研判的数据，另一个页签处理下发文件中无法直接确定去向的数据；两类记录继续使用各自原有数据、权限和保存流程。“工单中心”位于工作台，调照片处理队列使用独立页面。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
