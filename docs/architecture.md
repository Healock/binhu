...
> 当前基线（v0.19.11）：平台仍使用同一个 MySQL 实例和八个业务域数据库；调照片名单支持腾讯新增申请增量进入平台、平台快捷申请提交后立即异步写回腾讯、完整核对安全重定位，以及腾讯批次结单后继续通过 ZIP 补挂真实照片。工单附件查看与管理权限已经分离：有权访问工单的业务人员可预览和下载附件，只有处理岗位和工单管理员可上传或删除。小区管理支持组长、组员维护和导出本人社区资料，并在页面标题区提供可见的导出和新增入口；已处理下发文件中的已注销对象按文件社区重新下发核查，全链条发布同时兼容含“登记情况”的 15 列表和旧 14 列表。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
...
电脑端在线汇总和 Univer 查询保持不变，同时在侧栏增加“指令核查”，从 `/tasks/home` 进入与手机端相同的任务首页、列表和详情。任务列表按可读宽度自适应排列，宽屏通常显示约六列；卡片顶部显示姓名、首个手机号和右上角状态，多号码以“首号 +N”通过系统原生选择器复制或拨打。中间仅用中性色展示可复制身份证号和两行地址，研判结果及来源标签保留语义色；社区、核查人和首次下发日期进入底部中性深色信息带。详情摘要直接显示当前腾讯来源行的身份证号、手机号、来源、原地址和现住址，全部原始字段继续收起供核对。所有查看、分配和回写规则与手机端共用后端校验。片长、基础管控、中队长、所队领导、管理员、超级管理员和叠加对应管理员权限组的账号也能进入这个入口，首页和列表使用其岗位查看范围；这类账号的手机 Dock 使用独立 `flow_tasks` 页面项，因此可以同时保留 Univer“在线查询”。能查看不代表一定能修改，回写仍由账号的 `online.raw.edit`、岗位硬限制、总开关、来源版本和行哈希共同决定。
