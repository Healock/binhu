...
- 内勤上传的公安全链条 `.xls/.xlsx`：先形成审核批次，人工确认后再向腾讯全链条表发布。
...
后端接口为 `/api/mobile-tasks/home`、列表、详情和来源行批量更新四类。每个接口都重新检查关联人员
岗位、唯一社区、功能权限和社区别名范围；权限组可以扩大查看能力，但不能突破组长、组员只能编辑
本人社区的岗位硬限制。常规业务的结果字段为空才算未完成，已有现住址但结果为空为“待补结果”；
...
公安地址库把“各小区”和“滨湖公寓明细”合并成长期数据源。居民小区和公寓都只负责地址到正式社区
的映射，公寓不能因为名称或模式自动算作酒店。导入时空白社区向下继承；社区只能通过社区管理中的
正式名称和别名解析。同名同社区合并来源，同名跨社区或高度相似但归属不同的记录进入冲突列表。
...
地址、别名、批次和已完成任务继续可读。有人员关联或未完成公安任务时拒绝停用；有公安地址或历史
...
Univer 查询不变，组长、组员的原手机任务工作台也不变。单条与批量审核都携带任务版本，其他人已经
