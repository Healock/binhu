...
> 当前基线（v0.19.12）：平台仍使用同一个 MySQL 实例和八个业务域数据库；任务处理下统一提供“下发任务处理、研判、调照片”三个入口，已处理文件确认导入后的人工复核记录仍保存在原批次并直接进入待研判视图，未完成全批审核前不能发布。“工单中心”移入工作台，继续承载发起人和通用工单视图；调照片处理队列则使用独立页面。调照片名单支持腾讯新增申请增量进入平台、平台快捷申请提交后立即异步写回腾讯、完整核对安全重定位，以及腾讯批次结单后继续通过 ZIP 补挂真实照片。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
