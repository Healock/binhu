...
在线数据汇总保留外部腾讯监控状态卡，同时把外部快照中的社区、下发日期和任务状态计数叠加到展示层的任务总数、结转、新下发、已有任务变化、待完成和已完成指标。社区统计表按社区合并并重新计算完成率；网格员统计表使用“外部腾讯表（只读）”标识行，不伪造具体网格员归属。外部计数只来源于脱敏业务桶，不写入本地任务表、`_daily_task_ledger`、工作量或派生业务数据；腾讯表仍不是平台业务主数据源，也不会被平台编辑回写。外部监控卡继续显示当前行数、新增、内容变化、移除和数据截至时间，其中变化指标累计所选日期内每轮读取相对上一成功快照的变化次数。读取失败保留上一成功状态并标记过期或失败，不能把失败解释成空表；腾讯异常不得阻塞本地汇总。
