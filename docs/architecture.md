...
- 管理员上传的走访明细 XLSX：校验、去重后保存，供后续走访汇总使用。
...
第一阶段的走访数据保存在 `OnlineData`，不写入另外两个数据库：

| 表 | 用途 |
|---|---|
| `_visit_import_batches` | 保存每次导入的文件摘要、日期范围和处理数量 |
| `t_visit_details` | 保存走访明细和用于去重的标准字段 |
...
“走访汇总”第一阶段只处理走访明细 XLSX，还没有接入星级评定表，也不会生成最终走访汇总。
...
4. 社区名称去掉末尾的“社区”二字。匹配不到现有社区时仍然入库，同时给出提醒。
5. 空白的房间核查数量、新增、变更和注销按 0 处理，填写的值必须是非负整数。

去重使用“本地业务日期 + 标准化地址”：

- 同一天同一地址只保留一条，以入户时间更晚的记录为准。
...
- 走访汇总页面：`frontend/src/pages/VisitSummary.tsx`。第一阶段只显示数据范围、上传结果和导入问题。
