...
- 各社区的完整登录账号在社区管理中分别加密保存且不回显；超级管理员在系统设置中多选后台只读查询范围，并配置外部基地址、统一密码和 MAC 服务。每条任务先解析正式社区，再使用该社区账号复用 `community_<id>` 加密会话；范围变化时清理被移除社区的旧会话。登录挑战在后台自动获取并提交空验证码。代码只允许挑战获取、登录、常住人口预检索、流动人口登记查询和人员照片查询五类固定上游路径，不提供登记、注销或任意转发能力。有登记资料的任务详情按需实时读取住址、年龄、户籍地址、民族、注销状态及照片，只返回白名单字段且不持久化人员正文或照片。详细合同见 [居住证平台首次登记查询](居住证平台首次登记查询.md)。
...
- 离线配置保存在当前客户端。在线配置读取只返回接口地址、MAC 服务地址、超时、启用状态，以及系统设置当前所选社区的 ID、名称和非敏感组织代码；完整账号、统一密码和后台会话令牌都不通过普通接口返回。成功读取后，客户端按社区建立独立账号槽位并保留仍在范围内的本机账号；没有本地账号或密码时必须由用户在离线配置面板逐社区手动填写。
