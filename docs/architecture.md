...
`online_summary` 导航配置 ID。侧栏和手机 Dock 中的“首页”固定在第一项，旧个性化配置读取后会自动
补齐该入口，用户不能隐藏。`/query` 原有的手机任务分流继续保留，完整流口任务和下发任务工作台仍分别
...
- 手机 `/query` 自动转到 `/tasks`；Dock 新增固定 `dashboard` 首页，原 `online_summary` 配置 ID 迁到 `/summary`，`online_query` 继续兼容旧配置并按岗位显示“任务处理”。
...
不满足要求时，脚本自动恢复这些应用材料。数据库备份只会创建和校验，不会被脚本导入；数据库恢复、外部写入
