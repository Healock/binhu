...
> 当前基线（v0.19.19）：平台仍使用同一个 MySQL 实例和八个业务域数据库；任务处理下统一提供“下发任务处理、研判、调照片”三个入口。“研判”可切换待研判与已研判，并使用独立详情和保存接口。`online.task.manage` 只在流口任务工作台内授予全所查看、现有任务编辑和核查人分配，不开放 Univer 或腾讯原始行增删。业务和管理列表使用统一的即时筛选工具栏。“工单中心”位于工作台，调照片处理队列使用独立页面。快捷调照片工单完成后，原核查任务详情直接投影可访问的照片附件，完成通知可返回该任务。已处理数据导入后可直接从待发布工作台整批发布校验通过的任务，异常记录留在原批次继续复核；下发卡片复用流口任务卡片的信息层级。`0.19.19` 修复 Univer 对18位身份证等长数字的显示精度损失，并在腾讯回写端强制把身份证和电话类字段保存为文本。数据库迁移、容器重建等底层维护仍由 Nginx 维护页承接，旧表和迁移备份暂不删除。
