...
下拉选项、冻结表头和自然空白行。腾讯来源值以强制文本单元格载入，确保日期、身份证号和
长手机号不会被 Univer 再次解释为数字。腾讯范围读取接口对数值单元格只返回浮点值，不提供原始输入或
...
最小宽度，地址、反馈等长字段设有最大宽度。全屏时由文档根节点进入浏览器全屏，表格卡片再覆盖整个视口，
...
组长、组员在小于 768px 的手机视口访问首页时，不再进入原手机在线汇总：

- `/` 显示姓名、社区、岗位、最近同步时间，以及本人待核查、今日新下发、昨日结转、今日完成和本社区待核查。
- `/query` 自动转到 `/tasks`；Dock 继续复用原 `online_summary` 和 `online_query` 配置 ID，只把手机文案改成“首页”和“任务处理”。
...
电脑端在线汇总和 Univer 查询保持不变，同时在侧栏增加“任务处理”，从 `/tasks/home` 进入与手机端相同的任务首页、列表和详情。列表与业务卡在桌面采用双列布局，所有查看、转派和回写规则与手机端共用后端校验。管理员和超级管理员也能进入这个入口，首页和列表固定显示“全所”范围；管理员能查看不代表一定能修改，回写仍由账号的 `online.raw.edit`、岗位硬限制、总开关、来源版本和行哈希共同决定。
...
预处理顺序是：酒店、宾馆和民宿建议无需登记；地址库、正式社区名或移交备注唯一命中时建议相应社区；
...
地址建议、平均分配和重复组，清除受影响且尚未发布任务的旧审核。姓名、身份证号、地址或启用社区
缺失时不能下发，仍可人工确认为无需登记、移交或重复排除。普通审计只保存字段名和内容摘要哈希。

基础管控和中队长在手机端访问 `/` 或 `/query` 会进入共享的 `/police-tasks` 队列；电脑端原首页和
Univer 查询不变，并在侧栏增加“下发任务处理”进入同一队列。电脑使用双列任务卡和右侧审核抽屉，手机
...
- 结转、新下发、已有任务变化、待完成和已完成卡片可以打开分页明细。明细接口和卡片共用同一分类函数、固定人员范围及社区权限；对象信息从该任务在区间内最后一次活动对应的历史快照读取，不能拿当前在线投影替代历史数据。

概览中的可用日期范围来自 `_daily_task_ledger_runs`，表示已经生成任务流水的日期，
不能拿没有同步快照的日期补造数据。明细按业务、任务键和区间最终状态去重，分页响应中的
...
- 全链条腾讯来源表支持两种物理布局：旧版 14 列，以及在“地址”和“创建时间”之间增加“登记情况”的 15 列。同步、查询回写和下发发布都会读取真实表头选择布局；“登记情况”作为来源占位列保留，正式业务值仍只包含原 14 个字段，避免创建时间、现住址、核查结果、研判和二次反馈整体错位。
...
| `backend/routers/mobile_tasks.py` | 提供流口岗手机首页、任务列表、详情和多字段安全回写接口 |
...
其次使用账号显示姓名，账号已删除时回退到记录发生时保存的用户名。页面默认只展示这些可读字段，
...
- 响应式显示：组长、组员手机首页和在线查询改用任务工作台；其他岗位的在线汇总手机端仍使用精简列表并保留完整表格入口。走访汇总、社区管理和用户管理手机端使用卡片。页面不再读取旧的表格/卡片偏好。
