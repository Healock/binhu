...
- 状态保存在 `OnlineData._residence_registration_status`，只包含任务定位、身份证 HMAC、状态、安全错误码和时间，不保存身份证、人员资料或外部响应正文。身份证或来源发生变化以及结果超过7天时重新查询，服务重启可恢复中断的只读项目。
- 外部基地址、凭据、MAC 服务和机构代码由超级管理员在系统设置中配置。代码只允许验证码、登录、常住人口预检索和流动人口登记查询四类固定路径，不提供登记、注销或任意转发能力。详细合同见 [居住证平台首次登记查询](居住证平台首次登记查询.md)。
