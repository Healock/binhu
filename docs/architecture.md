...
- 各社区的完整登录账号在社区管理中分别加密保存且不回显；超级管理员在系统设置中选择后台登录社区，并配置外部基地址、统一密码和 MAC 服务。系统按所选社区账号复用加密会话；切换社区或修改账号后清理旧会话。登录挑战在后台自动获取并提交空验证码。代码只允许挑战获取、登录、常住人口预检索、流动人口登记查询和人员照片查询五类固定上游路径，不提供登记、注销或任意转发能力。有登记资料的任务详情按需实时读取住址、年龄、户籍地址、民族、注销状态及照片，只返回白名单字段且不持久化人员正文或照片。详细合同见 [居住证平台首次登记查询](居住证平台首次登记查询.md)。
