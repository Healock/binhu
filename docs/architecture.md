...
登录页使用公安业务品牌的响应式双栏布局：桌面左侧叠放蓝色网格底图与透明城市丝绸线稿，展示“数智赋能、守护平安滨湖”；右侧使用透明警徽素材和“滨湖公安智慧平台”名称承载原登录表单。宽度不超过 900px 时隐藏装饰侧栏，保留警徽、平台名称、错误提示和完整登录表单。登录页视觉不随用户上次保存的深浅色主题改变，避免未登录状态下出现浅色背景与深色表单控件混用。
