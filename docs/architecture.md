...
- 状态保存在 `OnlineData._residence_registration_status`，只包含任务定位、身份证 HMAC、状态、安全错误码、单条查询总耗时和时间，不保存身份证、人员资料或外部响应正文。保存并开启配置后立即处理已有待查询任务，后台每60秒检查新增/变化任务；身份证或来源发生变化以及结果超过7天时重新查询，服务重启可恢复中断的只读项目。
