...
多实例通过 `daily_report` 命名锁串行执行监控。每个目标设置总行数、单表超时和固定读取周期；请求量继续进入去标识的腾讯 API 用量统计。监控代码只能调用文件信息、表头和范围读取接口，不能调用批量更新、清空、增表、删行或其他写接口。Dev、Staging 和 Shadow 的环境门禁固定禁止启用该外部监控。
