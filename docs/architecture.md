...
- 外部基地址、统一密码和 MAC 服务由超级管理员在系统设置中配置。账号按任务社区使用 `全民防社区代码 + 00` 自动生成，各社区令牌独立加密缓存；登录挑战在后台自动获取并提交空验证码。代码只允许挑战获取、登录、常住人口预检索、流动人口登记查询和人员照片查询五类固定上游路径，不提供登记、注销或任意转发能力。有登记资料的任务详情按需实时读取住址、年龄、户籍地址、民族、注销状态及照片，只返回白名单字段且不持久化人员正文或照片。详细合同见 [居住证平台首次登记查询](居住证平台首次登记查询.md)。
...
- 离线配置保存在当前客户端。在线配置同步只返回接口地址、MAC 服务地址、超时、启用状态和启用社区代码列表；统一密码和后台会话令牌不通过普通接口返回。没有本地密码时必须由用户在离线配置面板手动填写。
