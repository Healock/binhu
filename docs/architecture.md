...
“分配数据”进入独立全屏工作台。工作台只读取当前范围内未分配、未完成且无来源冲突任务的行键、社区、来源和地址，不返回姓名、身份证号、手机号或可编辑正文；候选按社区分组、按地址排序，支持点击/拖动连续选择、全选本社区、指定核查人和按社区平均分配剩余数据。分配成功后候选立即消失，关闭工作台不会提交未确认选择；每次写回仍沿用 20 条分块、来源版本校验和既有批量分配审计。
