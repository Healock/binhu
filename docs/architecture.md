...
> 当前基线（v0.21.0 开发分支）：平台仍使用同一个 MySQL 实例和八个业务域数据库；既有任务处理、流口工作台、走访来源、告知书读取和固定发布结构保持不变。全民防模型三在已验收只读预演上增加仅账号 `shenshenghua` 可用的单条“在吴”封闭真实登记，采用重新预演、短时准备记录、固定确认短语、二次确认、九步状态、失败冻结和最终复核。真实登记与写入协议开关默认关闭；超级管理员只负责配置，不能获得执行权限。运行记录进入 `PlatformData`，只保存安全摘要和步骤状态；人员正文、照片、密码和设备值不落库。该开发分支只提交 PR，尚未执行真实登记、合并、部署或创建标签。
