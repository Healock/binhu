...
双向同步采用“本地工单优先、外部写回排队”：在线任务快捷申请先提交工单，再由 `photo_sheet_outbox` 幂等追加腾讯 A–F；照片成功入库或工单确认“找到”后，本地状态和通知先完成，G 列的 `滨湖平台已调照片` 独立重试。读取开关与写入开关分离，出站写入还必须同时满足全局在线回写开关。

主要表为 `photo_sheet_sources`、`photo_sheet_batches`、`photo_sheet_rows`、`photo_sheet_outbox`、`photo_sheet_sync_runs` 和 `photo_sheet_conflicts`。每五分钟读取末尾重叠窗口，每天做一次完整只读重定位；腾讯物理行移动时使用 A–F 摘要定位，候选不唯一就进入冲突列表，不猜测、不覆盖。
