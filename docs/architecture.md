...
> 当前基线（v0.21.1 开发分支）：平台仍使用同一个 MySQL 实例和八个业务域数据库；既有任务处理、流口工作台、走访来源、告知书读取和固定发布结构保持不变。全民防模型三仅向账号 `shenshenghua` 提供一个“全民防登记”入口，点击后由后端自动完成登记前四步读取与校验，再进入短时准备、固定确认短语、二次确认、九步状态、失败冻结和最终复核。运行记录进入 `PlatformData`，只保存安全摘要和步骤状态；人员正文、照片、密码和设备值不落库。
