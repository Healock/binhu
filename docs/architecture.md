...
- 外部基地址、统一密码和 MAC 服务由超级管理员在系统设置中配置。账号按任务社区使用 `全民防社区代码 + 00` 自动生成，各社区令牌独立加密缓存；登录挑战在后台自动获取并提交空验证码。代码只允许挑战获取、登录、常住人口预检索和流动人口登记查询四类固定上游路径，不提供登记、注销或任意转发能力。详细合同见 [居住证平台首次登记查询](居住证平台首次登记查询.md)。
