...
- `POST /api/qmf-registration/runs/{id}/execute` 只接受固定短语“确认登记”。领取动作由数据库级锁串行化，并同时检查执行中运行和安全幂等键，防止多进程或重复准备造成二次登记。
- 写客户端只实现 `uploadPhoto`、`saveLocalPhoto`、`addPeople`、`fnmxCheck` 四个固定路径。执行前重新读取腾讯来源和上游任务；固定九步中任一步超时、断线、业务失败或结果不确定都会冻结，不自动重试或继续。
...
- `PlatformData._qmf_registration_runs` 保存来源版本/摘要、业务主键和上游任务 HMAC、步骤状态、结果代码、图片类型/大小/摘要及时间，不保存人员正文、照片、密码、IMEI 或 MACHINEUID。执行中断在下次启动时转为不确定。
