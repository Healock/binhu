...
在线数据汇总分别展示平台本地任务指标与外部腾讯监控指标。外部指标只有当前行数、新增、内容变化、移除和数据截至时间，其中变化指标累计所选日期内每轮读取相对上一成功快照的变化次数。它们不能进入任务总数、工作量、待完成、已完成、核查率、见底率或 `_daily_task_ledger`。读取失败保留上一成功状态并标记过期或失败，不能把失败解释成空表；腾讯异常不得阻塞本地汇总。
