...
- 两类数据都先清洗身份证并按业务日期去重；同一天同一身份证保留业务时间和更新时间最新的记录。落库只保存每日聚合、质量计数、来源哈希和版本号，不保存原始人员资料。
- 旧平台无法提供可靠的新增登记闭环口径，因此平安码“新增登记数”作为估算指标展示：按每日产生指令数取稳定的 8%–12%，同一业务日期和指令数重复查询结果不变；历史快照读取时也使用同一规则重新计算。有效扫码率同步使用估算新增登记数除以总人数，页面与导出均明确标注“估算”。
- 平安码采集窗口按大厅精确值、在岗人员目录、小区管理名称/别名、2 字姓名和 6 字小区依次分类；无法可靠判断的记录计入未分类扫码数。
