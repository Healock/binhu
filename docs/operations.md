...
- 导出的 DOCX 不保存在服务器，草稿保存在 `OnlineData._work_log_drafts`。
- 业务日期为 7 月 14 日时，取 7 月 14 日数据，文件名以 `0714日报` 开头，首页落款为 7 月 15 日。

修改母版时只运行 `backend/scripts/build_work_log_template.py` 生成空白母版。旧 `.doc` 样例只能在本机参考，不能复制进仓库。发布前要解压检查 DOCX：必须有 12 个内嵌 `.xlsx`、12 个预览图、没有未替换占位符，也不能搜到旧样例中的姓名、身份证号、电话号码或警情文字。

`0.7.0` 新增兼容表 `_work_log_drafts`，不删除或改写现有业务数据。部署前备份 `OnlineData` 并保留旧后端镜像和前端；部署后检查页面权限、草稿唯一键、自动保存冲突、DOCX 下载、12 个 Excel 对象、容器日志和 `/api/health` 版本。
