...
- Staging 新运行编号下的 75 人/30 分钟真实前端请求模型复测及清理。
...
### Staging 75 客户端、30 分钟真实前端请求模型复测
...
30 分钟窗口内使用 75 个并发客户端分别运行核心业务、自动轮询、SSE/Query WebSocket 和带随机抖动的失败退避。除登录、列表、详情、保存、领取和分配外，还覆盖心跳、未读数、维护状态、身份检查、任务列表刷新、编辑器读取、Query 版本、房屋搜索、断线恢复和错峰重连；按业务、轮询、实时连接、MySQL、Redis、Kafka、Flink 和派生队列分层出具证据。
证据目录：
