...
export default function ReteTaskFlowLab() {
  ...
  return (
    <div className="rete-task-flow-lab mx-auto max-w-[1680px] space-y-4 pb-4">
      ...
      <Modal open={Boolean(preview)} title="个人任务图只读预览" footer={<Button type="primary" onClick={() => setPreview(null)}>知道了</Button>} onCancel={() => setPreview(null)}>{preview && <div className="task-graph-preview-grid"><div><span>当前投影</span><b>{preview.projection_rows}</b></div><div><span>无法核实</span><b>{preview.unable_to_verify}</b></div><div><span>已研判</span><b>{preview.analyzed}</b></div><div><span>预计依赖链</span><b>{preview.eligible_chains}</b></div><div><span>核查人为空</span><b>{preview.blank_inspector}</b></div><div><span>账号映射异常</span><b>{preview.unmatched_inspector}</b></div></div>}</Modal>
    </div>
  )
}
