...
try{
 ...
 for(let i=1;i<=23;i++){await page.getByRole('heading',{name:`虚构任务${String(i).padStart(2,'0')}`,exact:true}).waitFor();if(i===2){await page.locator('.address-saved').getByRole('button',{name:'查看'}).click();await page.getByRole('heading',{name:'虚构任务01',exact:true}).waitFor();await page.getByRole('button',{name:'返回刚才正在核对的任务'}).click();await page.getByRole('heading',{name:'虚构任务02',exact:true}).waitFor()}await page.getByRole('radio',{name:'虚构小区甲',exact:false}).check();await page.getByRole('button',{name:'确认并下一条',exact:true}).click()}
 ...
 await page.getByRole('heading',{name:'虚构任务23',exact:true}).waitFor()
 ...
 await page.goto('http://127.0.0.1:5197/tests/addressWorkbench.fixture.html?parser_type=全链条')
 delayedId=2
 await page.locator('.address-queue-row').filter({hasText:'虚构任务02'}).click()
 await page.locator('.address-queue-row').filter({hasText:'虚构任务03'}).click()
 await page.getByRole('heading',{name:'虚构任务03',exact:true}).waitFor()
 await page.waitForTimeout(700)
 assert.equal(await page.locator('.address-review-heading h2').innerText(),'虚构任务03')
 delayedId=0
