...
test('Android client locks portrait and forces the packaged frontend into mobile mode', () => {
  ...
  assert.match(viteConfig, /targets: \['Chrome >= 51'\]/)
  assert.match(viteConfig, /modernTargets: \['Chrome >= 61'\]/)
  assert.match(viteConfig, /android-css-compatibility/)
  assert.match(viteConfig, /flattenLayers/)
  assert.match(startupGuard, /Android System WebView\/Chrome/)
  assert.match(startupGuard, /浏览器内核/)
  assert.match(startupGuard, /window\.location\.reload/)
  assert.ok(html.indexOf('/startup-guard.js') < html.indexOf('/legacy-polyfills.js'))
  assert.match(html, /id="binhu-startup"/)
  ...
  assert.match(styles, /html\.native-mobile-shell a\s*\{[\s\S]*?text-decoration:\s*none;/)
  assert.match(styles, /html\.native-mobile-shell \.app-sidebar__nav a\s*\{[\s\S]*?display:\s*flex;/)
  assert.match(styles, /--color-slate-600:\s*#475569;/)
