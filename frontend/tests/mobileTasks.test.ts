...
test('流口任务支持账号级表格视图并在手机端保留卡片', () => {
  ...
  assert.match(styleSource, /mobile-task-table-primary-row:not\(:first-child\)[\s\S]*border-top: 2px solid var\(--app-border-strong\)/)
  assert.match(styleSource, /ant-table-expanded-row[\s\S]*border-bottom: 10px solid var\(--app-bg\)/)
