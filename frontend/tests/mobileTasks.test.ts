...
test('任务卡片使用可读密度、完整身份证主体和来源标签云', () => {
  ...
  assert.match(pageSource, /createResilientPoller\(refreshVisibleList/)
  assert.match(pageSource, /intervalMs: 30_000/)
  assert.match(pageSource, /failureThreshold: 4/)
  assert.match(pageSource, /poller\.stop\(\)/)
  assert.doesNotMatch(pageSource, /setInterval\(refreshVisibleList, 30_000\)/)
...
test('地址匹配状态不阻断已归属社区的人工分配', () => {
  ...
  assert.match(tableSource, /AddressStatusTag/)
  assert.doesNotMatch(tableSource, /confirmMobileTaskAddressMatch/)
  assert.doesNotMatch(tableSource, /mobile-task-address-match/)
  assert.match(detailSource, /AddressStatusTag/)
  assert.doesNotMatch(detailSource, /mobile-task-address-match/)
...
test('指令核查编辑器防抖与失焦共用队列并提供失败重试', () => {
  ...
  assert.match(detailSource, /task_revision_conflict/)
  assert.match(detailSource, /current_values/)
  assert.match(detailSource, /重试保存/)
