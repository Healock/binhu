...
test('唯一可靠建议作为自动匹配直接参与分配', () => {
  ...
  assert.match(detailSource, /按候选社区修正并重新匹配/)
  assert.match(detailSource, /请联系基础管控或管理员处理/)
  assert.match(detailSource, /下一步：联系基础管控或管理员处理地址冲突/)
  ...
  assert.match(pageSource, /需要处理[\s\S]*条[\s\S]*问题明细/)
