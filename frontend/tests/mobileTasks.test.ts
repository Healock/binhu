...
test('流口任务支持账号级表格视图并在手机端保留卡片', () => {
  ...
  assert.match(tableSource, /编辑本行/)
  assert.match(tableSource, /getMobileTaskDetail/)
  assert.match(tableSource, /updateMobileTask/)
  assert.match(tableSource, /mobileTaskEditorFields/)
  assert.match(tableSource, /保存 \$\{Object\.keys\(editorChanges\)\.length\} 项/)
  ...
  assert.match(styleSource, /mobile-task-table-primary-row:not\(:first-child\)[\s\S]*border-top: 1px solid var\(--app-border-strong\)/)
  assert.match(styleSource, /ant-table-expanded-row[\s\S]*border-bottom: 8px solid var\(--app-bg\)/)
