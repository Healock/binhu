...
test('已处理数据在任务工作台多选发布且卡片沿用流口任务规范', () => {
  ...
  assert.match(panelSource, /仅导入，稍后发布/)
  assert.match(panelSource, /getPoliceDispatchPublishableSelection/)
  assert.match(panelSource, /publishSelectedPoliceDispatchTasks/)
  assert.match(panelSource, /pending_review&category=manual/)
  assert.match(panelSource, /留待研判/)
  assert.doesNotMatch(panelSource, /整批发布/)
  assert.match(panelSource, /查看处理进度/)
  assert.match(panelSource, /手机号缺失等异常记录会留在“未下发数据研判”/)
