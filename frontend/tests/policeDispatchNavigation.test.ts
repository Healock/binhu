...
test('下发状态筛选并入批次统计并显示分组数量', () => {
  ...
  assert.match(source, /const reconciliationHint = '本地发布结果仍需核对/)
...
test('已处理数据在任务工作台多选发布且卡片沿用流口任务规范', () => {
  ...
  assert.match(panelSource, /getPoliceImportProfiles/)
  ...
  assert.match(workbenchSource, /本地发布结果仍需核对/)
  assert.match(workbenchSource, /本地现有内容与待发布内容不同/)
  assert.doesNotMatch(workbenchSource, /腾讯内容|腾讯来源|腾讯写入/)
...
test('业务数据导入支持多业务表快捷下发并仍由发布工作台落表', () => {
  ...
  assert.match(panelSource, /提交后只加入本地下发任务池，请在发布工作台确认后发布/)
  assert.doesNotMatch(panelSource, /腾讯目标表|写入腾讯表/)
  assert.match(panelSource, /createQuickPoliceDispatch/)
  assert.match(panelSource, /const quickSubmittingRef = useRef\(false\)/)
  assert.match(panelSource, /if \(quickSubmittingRef\.current\) return/)
  assert.match(panelSource, /quickSubmittingRef\.current = true[\s\S]*createQuickPoliceDispatch/)
  assert.match(panelSource, /request_id: quickRequestIdRef\.current/)
  assert.match(panelSource, /key: 'quick-dispatch-submit'/)
  assert.match(panelSource, /getQuickDispatchOptions/)
  assert.match(panelSource, /status=pending_publish&category=all/)
  assert.match(clientSource, /api\.post\('\/police-dispatch\/quick-dispatch'/)
  assert.match(clientSource, /request_id: string/)
