...
test('任务分配和行内编辑在紧凑桌面宽度保持完整可操作', () => {
  ...
  assert.match(styles, /\.mobile-task-table-inline-field > span\s*\{[^}]*color:\s*var\(--app-text-strong\);[^}]*font-size:\s*13px;[^}]*font-weight:\s*750;/s)
  assert.match(taskTableSource, /resultNeedsSecondaryFollowup[\s\S]*?includes\('无法核实'\)/)
  assert.match(taskTableSource, /hasSecondaryFeedback[\s\S]*?values\['二次反馈'\]/)
  assert.match(taskTableSource, /isSecondaryFeedback && resultNeedsSecondaryFollowup/)
  assert.match(taskTableSource, /二次核查完成后，请务必将“核查结果”修改为最终结果。/)
  assert.match(taskTableSource, /已填写二次反馈，请现在将“核查结果”修改为最终结果。/)
  assert.match(taskTableSource, /mobile-task-table-inline-field--attention/)
  assert.match(styles, /\.mobile-task-table-inline-guidance--active\s*\{[^}]*color:\s*var\(--app-warning\);[^}]*font-weight:\s*700;/s)
  assert.match(styles, /\.mobile-task-table-inline-field--attention \.ant-select-selector\s*\{[^}]*border-color:\s*color-mix\(in srgb, var\(--app-warning\) 58%, var\(--app-border\)\)/s)
