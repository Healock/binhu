...
test('任务流聚合现有待办、被动刷新并跳转原任务详情', () => {
  ...
  assert.match(source, /const flowNodes = useMemo<FlowNode\[\]>\(\(\) => \[/)
  assert.match(source, /\.\.\.laneNodes/)
  assert.match(source, /\.\.\.deckNodes/)
  assert.match(source, /\.\.\.displayedTaskNodes/)
  ...
  assert.match(source, /STACK_THRESHOLD = 6/)
  assert.match(source, /activeCardByLane/)
  assert.match(source, /rotateCard/)
  assert.match(source, /上一张/)
  assert.match(source, /下一张/)
  assert.match(source, /打开处理/)
  assert.match(source, /processSteps/)
  assert.match(source, /收到任务/)
  assert.match(source, /审核与处理/)
  assert.match(source, /紧凑/)
  assert.match(source, /comfortable/)
  assert.match(source, /dragHandle: '\.task-flow-node__drag-handle'/)
  assert.match(source, /autoPanOnNodeDrag=\{false\}/)
  assert.match(source, /waitingForAnalysis \? '等待研判'/)
  assert.doesNotMatch(source, /NodeResizer/)
  assert.doesNotMatch(source, /drawTasks/)
  assert.doesNotMatch(source, /task-flow-workflow-/)
