...
test('任务流聚合现有待办、被动刷新并跳转原任务详情', () => {
  ...
  assert.match(source, /MAX_DRAWN_PER_LANE = 3/)
  assert.match(source, /抽取下一项/)
  assert.match(source, /全部收起/)
  assert.match(source, /task-flow-workflow-/)
  assert.match(source, /收到任务/)
  assert.match(source, /审核与处理/)
  assert.match(source, /task-flow-system-/)
  assert.match(source, /visibleNodes\.filter\(node => \(/)
  assert.match(source, /调整区域/)
  assert.match(source, /laneSizes/)
